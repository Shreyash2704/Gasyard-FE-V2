...
const Explorer = (props: Props) => {
  const [initailTxns, setInitailTxns] = useState<TxObjectArrayType | null>(
    null
  );
  ...
  const [debouncedValue, setDebouncedValue] = useState<`0x${string}` | string>(
    inputAddress
  );
  ...
  const getFilteredData = async (
    page: number,
    inputAddress: `0x${string}` | "" | string,
    chain1: chains_type | null,
    chain2: chains_type | null
  ) => {
    const response = await getListTransactions(
      page,
      inputAddress === "" ? null : inputAddress,
      chain1 && chain1.id ? chain1.id : null,
      chain2 && chain2.id ? chain2.id : null
    );
  ...
  const formatDate = (dateString: string) => {
    ...
    const formattedTime = `${String(hours).padStart(2, "0")}:${String(
      minutes
    ).padStart(2, "0")}:${String(seconds).padStart(2, "0")} ${ampm} UTC`;
        ...
        <div className="filters dflex-row">
          <div className="filter-chain1 dflex-row">
            ...
            <Menu>
              ...
              <MenuList>
                <MenuItem key={0} onClick={() => setchain1(null)}>
                  All Chains
                </MenuItem>
                {Chains.map((chain) => {
                  ...
                  return (
                    <MenuItem
                      key={chain.id}
                      onClick={() =>
                        onSelectChain(1, { id: chain.id, name: chain.name })
                      }
                    >
                      {chain.name}
                    </MenuItem>
          ...
          <div className="filter-chain1 dflex-row">
            ...
            <Menu>
              ...
              <MenuList>
                <MenuItem key={0} onClick={() => setchain2(null)}>
                  All Chains
                </MenuItem>
                {Chains.map((chain) => {
                  if (chain.id === 1) return <></>;
                  return (
                    <MenuItem
                      key={chain.id}
                      onClick={() =>
                        onSelectChain(2, { id: chain.id, name: chain.name })
                      }
                    >
                      {chain.name}
                    </MenuItem>
                      ...
                      {item.managerHash ? (
                        <>
                          ...
                          <img
                            ...
                            className="redirect"
                            onClick={() =>
                              redirectManagerHash(item.managerHash)
                            }
                          />
                        </>
                      ) : (
                      ...
                      <img
                        src={
                          item.inputChainID === 1802203764
                            ? iconMap[920637907288165]
                            : iconMap[item.inputChainID]
                        }
                        className="logo"
                      />
                      {item.inputChainID === 1802203764 ? (
                        <>
                          {formatToken(formatEther(item.inputChainAmount))}{" "}
                          {ChainJsonData[920637907288165].baseToken}
                        </>
                      ) : (
                        <>
                          {formatToken(formatEther(item.inputChainAmount))}{" "}
                          {ChainJsonData[item.inputChainID].baseToken}
                        </>
                      )}
                  ...
                  <td>
                    <div className="dflex-row chain">
                      ...
                      {item.outputChainID === 1802203764 ? (
                        ...
                        <>
                          {formatToken(formatEther(item.outputChainAmount))}{" "}
                          {ChainJsonData[item.outputChainID].baseToken}
                        </>
                      )}
